...
module overmind::slice_the_jpeg {
    ...
    use aptos_framework::fungible_asset::{Self};  // https://github.com/aptos-labs/aptos-core/blob/3d281ad455bd5881c41ae86cd7982b1fa2092cbf/aptos-move/framework/aptos-framework/doc/fungible_asset.md#module-0x1fungible_asset
    ...
    fun init_module(account: &signer) {
        ...
        let (resource_signer, resource_singer_cap) = account::create_resource_account(account, SEED);
        ...
        let nf_token_collection_address = create_split_collection(&resource_signer);

        // TODO: Create a new State object and send it to account
        move_to(
            account,
            State {
                signer_capability: resource_singer_cap,
                collection_address: nf_token_collection_address,
    ...
    public entry fun split<NFT: key >(
        ...
        let nf_token_owner_address = signer::address_of(nf_token_owner_signer);
        check_if_account_owns_nft(nf_token_owner_address, nf_token_object);
        ...
        let ft_token_address = token::create_token_address(&resource_account_address, &string::utf8(SPLIT_COLLECTION_NAME), &split_token_name);
        check_if_split_token_does_not_exist(ft_token_address);

        // TODO: Transfer the NFT to the module's resource account
        object::transfer<NFT>(nf_token_owner_signer, nf_token_object, resource_account_address);
        ...
        create_split_token_as_fungible_token(
            ...
            split_token_supply,
            nf_token_address,
            nf_token_owner_address,
            token::collection_name(nf_token_object),
            token::name(nf_token_object),
        );

        // TODO: Mint the total supply of the new split tokens to the NFT owner
        // 
        // USE: Use the mint_internal function below
        mint_internal(object::address_to_object<SplitToken>(ft_token_address), nf_token_owner_address, split_token_supply);
        ...
        event::emit_event(
            ...
            SplitEvent{
                splitter_address: nf_token_owner_address,
                nft_address: nf_token_address,
                split_token_address: ft_token_address,
                split_token_supply,
                split_token_call_threshold,
                split_token_call_price,
            });
    ...
    public entry fun redeem<NFT: key>(
        ...
        check_if_nf_token_is_owned_by_account<NFT>(nf_token_address, resource_account_address);
        ...
        let nft_address = property_map::read_address<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_NFT_ADDRESS));
        let nft_object = object::address_to_object<NFT>(nft_address);
        object::transfer<NFT>(&resource_signer, nft_object, coin_owner_address);
    ...
    public entry fun call<NFT: key>(
        ...
        let split_token_supply = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_MAX_SUPPLY));
        let call_payment = call_price * (split_token_supply - split_token_balance);
        check_if_aptos_coin_balance_is_equal_to_or_greater_than_call_payment(coin::balance<AptosCoin>(coin_owner_address), call_payment);

        // TODO: Transfer the correct amount of AptosCoin from the module's resource account to the coin
        //       owner
        coin::transfer<AptosCoin>(coin_owner, resource_account_address, call_payment);
        ...
        event::emit_event(
            ...
            CallEvent{
                ...
                nft_address: nf_token_address,
                caller_split_token_amount: split_token_balance,
    ...
    public entry fun exchange_split_tokens_for_call_payment<NFT: key >(
        ...
        let resource_signer = account::create_signer_with_capability(&state.signer_capability);
        let resource_account_address = account::get_signer_capability_address(&state.signer_capability);
        let account_address = signer::address_of(coin_owner);
        ...
        let split_token_object = object::address_to_object<SplitToken>(split_token_address);
        let nf_token_address = property_map::read_address<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_NFT_ADDRESS));
        check_if_nf_token_is_not_owned_by_account<NFT>(nf_token_address, resource_account_address);
        ...
        let split_token_object = object::address_to_object<SplitToken>(split_token_address);
        let call_price = property_map::read_u64<SplitToken>(&split_token_object, &string::utf8(PROPERTY_NAME_CALL_PRICE));
        coin::transfer<AptosCoin>(&resource_signer, coin_owner_address, call_price * split_token_balance);
        ...
        event::emit_event(
            ...
            ExchangeEvent{
                exchanger_address: account_address,
                nft_address: nf_token_address,
                split_token_exchange_amount: split_token_balance,
                aptos_coin_exchange_payment: call_price,
            });
    }
    ...
    fun create_split_collection(creator: &signer): address {
        ...
        signer::address_of(&object::generate_signer(&collection_constructor_ref))
    }
    ...
    fun create_split_token_as_fungible_token(
        ...
        let construct_ref = token::create_named_token(creator, string::utf8(SPLIT_COLLECTION_NAME), description, name, option::none<aptos_token_objects::royalty::Royalty>(), uri);
        ...
        let property_map = property_map::prepare_input(
            ...
            vector[
                bcs::to_bytes<address>(&nft_address),
                bcs::to_bytes<u64>(&call_price),
                bcs::to_bytes<u64>(&call_threshold),
                bcs::to_bytes<u64>(&max_supply),
                bcs::to_bytes<String>(&nft_token_name),
                bcs::to_bytes<String>(&nft_collection_name),
                bcs::to_bytes<address>(&nft_creator_address),
            ]
        );
        ...
        let fungible_asset_mint_ref = fungible_asset::generate_mint_ref(&construct_ref);
        let fungible_asset_burn_ref = fungible_asset::generate_burn_ref(&construct_ref);
        ...
        move_to(&object_signer, split_token);
    }
    ...
    public fun split_balance(account: address, split_token: Object<SplitToken>): u64 {
        ...
        primary_fungible_store::balance(account, split_token)
    }
    ...
    fun mint_internal(split_token_object: Object<SplitToken>, receiver: address, amount: u64) acquires SplitToken {
        ...
        //
        // HINT: https://aptos.dev/standards/aptos-token-v2/#fungible-token
        let split_token_object_address = object::object_address(&split_token_object);
        let split_token_obj = borrow_global<SplitToken>(split_token_object_address);
        let store = primary_fungible_store::ensure_primary_store_exists(receiver, split_token_object);
        fungible_asset::mint_to(&split_token_obj.fungible_asset_mint_ref, store, amount)
    }
    ...
    fun burn_internal(owner: &signer, split_token_object: Object<SplitToken>, amount: u64) acquires SplitToken {
        ...
        // HINT: https://aptos.dev/standards/aptos-token-v2/#fungible-token
        let split_token_object_address = object::object_address(&split_token_object);
        let store = primary_fungible_store::primary_store(signer::address_of(owner), split_token_object);
        let split_token_obj = borrow_global<SplitToken>(split_token_object_address);
        fungible_asset::burn_from(&split_token_obj.fungible_asset_burn_ref, store, amount);
    }
    ...
    inline fun check_if_account_owns_nft<NFT: key>(account_address: address, nf_token_object: Object<NFT>) {
        assert!(object::owner(nf_token_object) == account_address, EAccountDoesNotOwnNft);
    }
    ...
    inline fun check_if_nf_token_is_owned_by_account<NFT: key>(nf_token_address: address, account_address: address) {
        let nft_token_object = object::address_to_object<NFT>(nf_token_address);
        assert!(object::owner(nft_token_object) == account_address, ENftIsNotOwnedByAccount);
    }
    ...
    inline fun check_if_nf_token_is_not_owned_by_account<NFT: key>(nf_token_address: address, account_address: address) {
        let nft_token_object = object::address_to_object<NFT>(nf_token_address);
        assert!(object::owner(nft_token_object) != account_address, ENftIsOwnedByAccount);
    }
