...
module overmind::broker_it_yourself {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let (resource_signer, resource_signer_cap) = create_resource_account(admin, SEED);
    ...
    */
    public entry fun create_offer(
        ...
        if (sell_apt) {
            ...
            coin::transfer<AptosCoin>(creator, account::get_signer_capability_address(&state.cap), apt_amount);
        };

        // TODO: Emit CreateOfferEvent event
        event::emit_event(&mut state.create_offer_events, broker_it_yourself_events::new_create_offer_event(next_offer_id, creator_address, arbiter, apt_amount, usd_amount, sell_apt, timestamp::now_seconds()))
    }
    ...
    */
    public entry fun accept_offer(user: &signer, offer_id: u128) acquires State {
        ...
        option::fill(&mut offer.counterparty, user_address);
        ...
        if (!offer.sell_apt) {
            ...
            coin::transfer<AptosCoin>(user, account::get_signer_capability_address(&state.cap), offer.apt_amount);
        };

        // TODO: Emit AcceptOfferEvent event
        event::emit_event(&mut state.accept_offer_events, broker_it_yourself_events::new_accept_offer_event(offer_id, user_address, timestamp::now_seconds()))
    }
    ...
    */
    public entry fun complete_transaction(user: &signer, offer_id: u128) acquires State {
        ...
        } else if (option::is_some(&offer.counterparty) && user_address == *option::borrow(&offer.counterparty)) {
            offer.completion.counterparty = true;
        };

        // TODO: Emit CompleteTransactionEvent event
        event::emit_event(&mut state.complete_transaction_events, broker_it_yourself_events::new_complete_transaction_event(offer_id, user_address, timestamp::now_seconds()));
        ...
        if (offer.completion.creator && offer.completion.counterparty) {
            ...
            simple_map::remove(&mut state.offers, &offer_id_copy);
            remove_offer_from_creator_offers(&mut state.creators_offers, &offer.creator, &offer_id_copy);
            ...
            event::emit_event(&mut state.release_funds_events, broker_it_yourself_events::new_release_funds_event(offer_id, user_address, timestamp::now_seconds()))
        }
    }
    ...
    */
    public entry fun cancel_offer(creator: &signer, offer_id: u128) acquires State {
        ...
        event::emit_event(&mut state.cancel_offer_events, broker_it_yourself_events::new_cancel_offer_event(offer_id, timestamp::now_seconds()))
    }
    ...
    */
    public entry fun open_dispute(user: &signer, offer_id: u128) acquires State {
        ...
        event::emit_event(&mut state.open_dispute_events, broker_it_yourself_events::new_open_dispute_event(offer_id, user_address, timestamp::now_seconds()))
    }
    ...
    */
    public entry fun resolve_dispute(
        ...
        let offer_id_ref = &offer_id;
        // TODO: Call assert_offer_exists function
        let state = borrow_global<State>(@admin);
        assert_offer_exists(&state.offers, offer_id_ref);

        // TODO: Call assert_dispute_opened function
        let offer = *simple_map::borrow(&state.offers, offer_id_ref);
        ...
        // TODO: Remove the offer from the list of available offers
        let state = borrow_global_mut<State>(@admin);
        simple_map::remove(&mut state.offers, offer_id_ref);

        // TODO: Remove the offer's id from the creator's offers list
        remove_offer_from_creator_offers(&mut state.creators_offers, &offer.creator, offer_id_ref);
    ...
    */
    ...
    public fun get_available_offers(): SimpleMap<u128, Offer> acquires State {
        ...
        let all_offers = borrow_global<State>(@admin).offers;
        let available_offers = simple_map::create<u128, Offer>();
        let (all_offer_ids,  _) = simple_map::to_vec_pair(all_offers);

        vector::for_each(all_offer_ids, |offer_id| {
            let offer_id_copy = offer_id;
            let offer = simple_map::borrow(&all_offers, &offer_id_copy);
    ...
    */
    ...
    public fun get_creator_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        if (simple_map::contains_key(&state.creators_offers, &creator)) {
            let creator_offer_ids = simple_map::borrow(&state.creators_offers, &creator);

            vector::for_each(*creator_offer_ids, |offer_id| {
                simple_map::add(&mut creator_offers, offer_id, *simple_map::borrow(&state.offers, &offer_id));
            });
        };
    ...
    */
    public(friend) inline fun remove_offer_from_creator_offers(
        ...
        let creator_offers = simple_map::borrow_mut(creators_offers, creator);
        let ( exists, offer_index ) = vector::index_of(creator_offers, offer_id);
        if (exists) {
            vector::remove(creator_offers, offer_index);
        }
    }
    ...
    */
    ...
    inline fun assert_user_participates_in_transaction(user: address, offer: &Offer) {
        ...
        assert!(user == offer.creator || (option::is_some(&offer.counterparty) && user == *option::borrow(&offer.counterparty)), ERROR_USER_DOES_NOT_PARTICIPATE_IN_TRANSACTION);
    }
    ...
    inline fun assert_user_has_not_marked_completed_yet(user: address, offer: &Offer) {
        ...
        } else if (option::is_some(&offer.counterparty) && user == *option::borrow(&offer.counterparty)) {
