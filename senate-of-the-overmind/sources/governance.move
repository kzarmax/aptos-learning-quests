module overmind::governance {
    ...
    */
    fun init_module(admin: &signer) {
        ...
        voting::register<GovernanceProposalType>(&resource_signer);
        ...
        collection::create_unlimited_collection(
            ...
            option::none<aptos_token_objects::royalty::Royalty>(),
        ...
        mint_governance_token(&resource_signer, string::utf8(ADMIN_STARTING_URI), admin_address, ADMIN_STARTING_VOTING_POWER, ADMIN_STARTING_PROPOSAL_ABILITY);
        ...
        let signer_caps = simple_map::create<GovernanceResponsibilityKey, SignerCapability>();
        let governance_responsibility_key = GovernanceResponsibilityKey {
            key: string::utf8(RESOURCE_ACCOUNT_SIGNER_CAP_KEY),
            description: string::utf8(RESOURCE_ACCOUNT_SIGNER_CAP_DESCRIPTION),
        };
        simple_map::add(&mut signer_caps, governance_responsibility_key, resource_cap);
        ...
        let governance_events = GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(&resource_signer),
            vote_events: account::new_event_handle<VoteEvent>(&resource_signer),
            updated_governance_config_events: account::new_event_handle<UpdateGovernanceConfigEvent>(&resource_signer),
            update_governance_responsibility_events: account::new_event_handle<UpdateGovernanceResponsibilityEvent>(&resource_signer),
            add_approved_execution_hash_events: account::new_event_handle<AddApprovedExecutionHashEvent>(&resource_signer),
            resolve_proposal_events: account::new_event_handle<ResolveProposalEvent>(&resource_signer),
        };
        
        // TODO: Move the 4 new global objects to the resource account
        move_to(&resource_signer, governance_config);
        move_to(&resource_signer, governance_responsibility);
        move_to(&resource_signer, approved_execution_hashes);
        move_to(&resource_signer, governance_events);
    }
    ...
    */
    public entry fun create_proposal(
        ...
        let governance_token_name = generate_governance_token_name(proposer_address);
        let proposer_governance_token_address = token::create_token_address(&resource_account_address, &string::utf8(GOVERNANCE_TOKEN_COLLECTION_NAME), &governance_token_name);
        check_if_address_is_an_object(proposer_governance_token_address);
        ...
        let governance_token_object = object::address_to_object<GovernanceToken>(proposer_governance_token_address);
        check_if_account_can_create_proposal(governance_token_object);
        ...
        let early_resultion_vote_threshold = generate_early_resolution_vote_threshold();

        // TODO: Create the new proposal
        //
        // HINT: Use the create_proposal_internal function below
        let proposal_id = create_proposal_internal(proposer_address, execution_hash, early_resultion_vote_threshold, proposal_metadata, is_multi_step_proposal);

        // TODO: Emit a new CreateProposalEvent
        let governance_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event(&mut governance_events.create_proposal_events,
            CreateProposalEvent{
                proposer: proposer_address,
                proposer_governance_token_address,
                proposal_id,
                execution_hash,
                proposal_metadata,
        });
    ...
    */
    public entry fun vote (
        ...
        let token_name = generate_governance_token_name(voter_address);
        let expected_governance_token_collection_address = token::create_token_address(
            &get_resource_account_address(),
            &string::utf8(GOVERNANCE_TOKEN_COLLECTION_NAME),
            &token_name,
        );
        check_if_address_is_an_object(expected_governance_token_collection_address);

        // TODO: Fetch the governance token object from the expected address
        let governance_token_object = object::address_to_object<GovernanceToken>(expected_governance_token_collection_address);
        ...
        check_if_voter_has_not_voted_on_proposal(proposal_id, governance_token_object);
        ...
        let governance_token = borrow_global<GovernanceToken>(expected_governance_token_collection_address);
        property_map::add(&governance_token.property_mutator_ref, proposal_id_key,                 string::utf8(b"u64"), std::bcs::to_bytes(&proposal_id));

        // TODO: Fetch the voter's voting power and execute the vote
        // 
        // HINT: Use the vote_internal function below 
        vote_internal(proposal_id, 1, should_pass);

        // TODO: Emit a VoteEvent
        let governance_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event(&mut governance_events.vote_events,
            VoteEvent{
                proposal_id,
                voter: voter_address,
                voting_power: ADMIN_STARTING_VOTING_POWER,
                should_pass,
            });
    }
    ...
    */
    public fun update_governance_config(
        ...
        let resource_account_signer_address = signer::address_of(resource_account_signer);
        check_if_account_is_resource_account_address(resource_account_signer_address);
        
        // TODO: Borrow the GovernanceConfig
        let governance_config = borrow_global_mut<GovernanceConfig>(resource_account_signer_address);
        ...
        let governance_events = borrow_global_mut<GovernanceEvents>(resource_account_signer_address);
        event::emit_event(&mut governance_events.updated_governance_config_events,
            UpdateGovernanceConfigEvent{
                old_minimum_voting_threshold: old_minimum_voting_threshold,
                old_voting_duration_seconds: old_voting_duration_seconds,
                new_mimimum_voting_threshold: minimum_voting_threshold,
                new_voting_duration_seconds: voting_duration_seconds,
            });
    }
    ...
    */
    public fun update_governance_responsibility(
        ...
        let resource_account_signer_address = get_resource_account_address();
        // TODO: Get the address of the resource_account_signer and ensure it is the actual resource account
        //
        // HINT: Use the check_if_account_is_resource_account_address function below
        check_if_account_is_resource_account_address(signer::address_of(resource_account_signer));

        // TODO: Update the GovernanceResponsibility's with the new key, description, and signer_cap
        let governance_responsibility = borrow_global_mut<GovernanceResponsibility>(resource_account_signer_address);
        let new_key = GovernanceResponsibilityKey {
            key, description
        };

        let (old_key, _) = simple_map::upsert(&mut governance_responsibility.signer_caps, new_key, signer_cap);
        let old_signer_key = if (option::is_none(&old_key)) { option::none<String>()} else {option::some(option::extract(&mut old_key).key)};
        let old_signer_description = if (option::is_none(&old_key)) { option::none<String>()} else {option::some(option::extract(&mut old_key).description)};

        // TODO: Emit the UpdateGovernanceResponsibilityEvent
        let governance_events = borrow_global_mut<GovernanceEvents>(resource_account_signer_address);
        event::emit_event(&mut governance_events.update_governance_responsibility_events,
            UpdateGovernanceResponsibilityEvent{
                old_signer_key,
                old_signer_description,
                new_signer_key: key,
    ...
    */
    public entry fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes, GovernanceEvents {
        ...
        let execuation_hash = voting::get_execution_hash<GovernanceProposalType>(resource_account_address, proposal_id);
        let approve_execution_hashes = borrow_global_mut<ApprovedExecutionHashes>(resource_account_address);
        simple_map::upsert(&mut approve_execution_hashes.hashes, proposal_id, execuation_hash);

        // Emit the AddApprovedExecutionHashEvent
        let governance_events = borrow_global_mut<GovernanceEvents>(resource_account_address);
        event::emit_event(&mut governance_events.add_approved_execution_hash_events,
    ...
    */
    public fun resolve(
        ...
        let governance_events = borrow_global_mut<GovernanceEvents>(resource_account_address);
        event::emit_event(&mut governance_events.resolve_proposal_events,
    ...
    */
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<GovernanceProposalType>(resource_account_address, proposal_id, next_execution_hash);
    ...
    */
    public fun mint_governance_token(
        ...
        let governance_token_description = generate_governance_token_description(soul_bound_to);

        // TODO: Generate the name for this new governance token
        // 
        // HINT: use the generate_governance_token_name function below
        let governance_token_name = generate_governance_token_name(soul_bound_to);
        ...
        let construct_ref = token::create_named_token(creator, collection_name, governance_token_description,  governance_token_name, option::none<aptos_token_objects::royalty::Royalty>(), uri);
        let token_address = token::create_token_address(&resource_account_address, &collection_name, &governance_token_name);
        ...
        object::transfer(creator, token_object, soul_bound_to);
        ...
        let property_mutator_ref = property_map::generate_mutator_ref(&construct_ref);
        let property_map = property_map::prepare_input(
            ...
            vector[
                std::bcs::to_bytes<u64>(&voting_power),
                std::bcs::to_bytes<bool>(&can_propose),
            ]
        );
        property_map::init(&construct_ref, property_map);

        // TODO: Create the GovernanceToken object and move it to the new token object signer
        let governance_token = GovernanceToken {
            mutator_ref: token::generate_mutator_ref(&construct_ref),
            burn_ref: token::generate_burn_ref(&construct_ref),
            property_mutator_ref,
        };
        move_to<GovernanceToken>(&object_signer, governance_token);
    }
    ...
    */
    inline fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        ...
        let approve_execution_hashes = borrow_global_mut<ApprovedExecutionHashes>(get_resource_account_address());
        assert!(simple_map::contains_key(&approve_execution_hashes.hashes, &proposal_id), EHashDoesNotExist);
        simple_map::remove(&mut approve_execution_hashes.hashes, &proposal_id);
    }
    ...
    */
    fun create_proposal_internal(
        ...
        //       duration
        let governance_config = borrow_global<GovernanceConfig>(get_resource_account_address());
        let expiration_timestamp =  timestamp::now_seconds() + governance_config.voting_duration_seconds;

        // TODO: Create the proposal in the aptos_framework::voting module
        //
        // HINT: Use voting::create_proposal_v2
        voting::create_proposal_v2(proposer, get_resource_account_address(), GovernanceProposalType{}, execution_hash, governance_config.minimum_voting_threshold, expiration_timestamp, early_resolution_vote_threshold, metadata, is_multi_step_proposal)
    }
    ...
    */
    fun vote_internal(
        ...
        voting::vote<GovernanceProposalType>(&GovernanceProposalType{}, resource_account_address, proposal_id, num_votes, should_pass);
    }
    ...
    */
    inline fun get_signer(key: String, description: String): signer acquires GovernanceResponsibility {
        ...
        let governance_responsibility = borrow_global<GovernanceResponsibility>(get_resource_account_address());
        ...
        let signer_cap = simple_map::borrow(&governance_responsibility.signer_caps, &governance_responsibility_key);
    ...
    */
    inline fun generate_proposal_id_key(proposal_id: u64): String {
        ...
        let str_proposal_id = string_utils::to_string_with_integer_types(&proposal_id);
        let proposal_id_key = string::utf8(BASE_PROPOSAL_ID_KEY);
        string::append(&mut proposal_id_key, str_proposal_id);
        proposal_id_key
    }
    ...
    */
    inline fun create_proposal_metadata(
        ...
        simple_map::add(&mut proposal_metadata, string::utf8(METADATA_LOCATION_KEY), metadata_location);
        simple_map::add(&mut proposal_metadata, string::utf8(METADATA_HASH_KEY), metadata_hash);
        proposal_metadata
    }
    ...
    */
    inline fun get_total_supply_of_governance_token(): Option<u64> {
        ...
        let governance_collection = object::address_to_object<Collection>(expected_governance_token_collection_address);

        // TODO: Return the count of the governance token collection
        collection::count<Collection>(governance_collection)
    }
    ...
    */
    inline fun generate_early_resolution_vote_threshold(): Option<u128> {
        ...
        let total_supply = get_total_supply_of_governance_token();
        ...
        if(option::is_some(&total_supply)) {
            option::fill(&mut early_resolution_vote_threshold, (*option::borrow(&total_supply) / 2 + 1 as u128))
        };
    ...
    */
    inline fun generate_governance_token_description(account_address: address): String {
        ...
        let str_account_address = string_utils::to_string_with_canonical_addresses(&account_address);
        let token_description = string::utf8(BASE_TOKEN_DESCRIPTION);
        string::append(&mut token_description, str_account_address);
        token_description
    }
    ...
    */
    ...
    inline fun check_if_voter_has_not_voted_on_proposal(
        ...
        assert!(property_map::contains_key(&governance_token, &proposal_id_key) == false, EVoterAlreadyVotedOnProposal);
    }
    ...
    inline fun check_if_proposal_voting_is_open(proposal_id: u64) {
        ...
        assert!(voting::is_voting_closed<GovernanceProposalType>(get_resource_account_address(), proposal_id) == false, EProposalVotingIsClosed);
    }
    ...
    inline fun check_if_account_can_create_proposal(governance_token: Object<GovernanceToken>) {
        ...
        assert!(property_map::read_bool(&governance_token, &string::utf8(GOVERNANCE_TOKEN_PROPOSAL_ABILITY_KEY)), EAccountDoesNotHaveProposalAbility);
    }
    ...
    inline fun check_if_proposal_can_be_resolved(proposal_id: u64) {
        ...
        assert!(voting::get_proposal_state<GovernanceProposalType>(get_resource_account_address(), proposal_id) == PROPOSAL_STATE_SUCCEEDED, EProposalCannotBeResolved);
    }
    ...
    inline fun check_if_address_is_an_object(object_address: address) {
        ...
        assert!(object::is_object(object_address), EAddressIsNotAnObject);
    }
    ...
    inline fun check_if_account_is_resource_account_address(account_address: address) {
        ...
        assert!(get_resource_account_address() == account_address, EAccountIsNotResourceAccount);
    }
    ...
    inline fun check_if_proposal_id_is_valid(proposal_id: u64) {
        ...
        assert!(proposal_id < voting::next_proposal_id<GovernanceProposalType>(get_resource_account_address()), EInvalidProposalId);
    }
