...
module overmind::split_or_steal {
    ...
    */
    fun init_module(account: &signer) {
        ...
        move_to(account, State {
            ...
            submit_decision_events: account::new_event_handle<SubmitDecisionEvent>(&resource_signer),
            reveal_decision_events: account::new_event_handle<RevealDecisionEvent>(&resource_signer),
            conclude_game_events: account::new_event_handle<ConcludeGameEvent>(&resource_signer),
            release_funds_after_expiration_events: account::new_event_handle<ReleaseFundsAfterExpirationEvent>(&resource_signer),
        });
    }
    ...
    */
    public entry fun create_game(
        ...
        let expiration_timestamp_in_seconds = timestamp::now_seconds() + EXPIRATION_TIME_IN_SECONDS;
        let game = Game {
            prize_pool_amount,
            player_one: PlayerData {
                player_address: player_one_address,
                decision_hash: option::none<vector<u8>>(),
                salt_hash: option::none<vector<u8>>(),
        ...
        simple_map::add(&mut state.games, next_game_id, game);
        ...
        coin::transfer<AptosCoin>(account, signer::address_of(&resource_signer), prize_pool_amount);

        // TODO: Emit `CreateGameEvent` event
        event::emit_event(&mut state.create_game_events ,CreateGameEvent{
    ...
    */
    public entry fun submit_decision(
        ...
        if (game.player_one.player_address == player_address) {
            option::fill(&mut game.player_one.decision_hash, decision_hash);
            option::fill(&mut game.player_one.salt_hash, salt_hash);
        } else {
            option::fill(&mut game.player_two.decision_hash, decision_hash);
            option::fill(&mut game.player_two.salt_hash, salt_hash);
        };

        // TODO: Emit `SubmitDecisionEvent` event
        event::emit_event(&mut state.submit_decision_events, SubmitDecisionEvent{
            game_id,
            player_address,
            decision_hash,
            salt_hash,
    ...
    */
    public entry fun reveal_decision(
        ...
        let player_address = signer::address_of(player);
        if (game.player_one.player_address == player_address) {
            player_data = &mut game.player_one;
        } else {
            player_data = &mut game.player_two;
        };

        let decision = make_decision(player_data, &salt);

        // TODO: Emit `RevealDecisionEvent` event
        event::emit_event(&mut state.reveal_decision_events, RevealDecisionEvent{
            game_id,
            player_address,
            decision,
            event_creation_timestamp_in_seconds: timestamp::now_seconds(),
        ...
        let game = *simple_map::borrow(&mut state.games, &game_id);
        let player_one_decision = game.player_one.decision;
        let player_two_decision = game.player_two.decision;

        if (player_one_decision != DECISION_NOT_MADE && player_two_decision != DECISION_NOT_MADE) {
            simple_map::remove(&mut state.games, &game_id);
            let prize_pool_amount = game.prize_pool_amount;

            if (player_one_decision == DECISION_SPLIT && player_two_decision == DECISION_SPLIT) {
                let half_pool_amount = prize_pool_amount / 2;
                coin::transfer<AptosCoin>(&resource_signer, game.player_one.player_address, half_pool_amount);
                coin::transfer<AptosCoin>(&resource_signer, game.player_two.player_address, half_pool_amount);
            } else if (player_one_decision == DECISION_SPLIT && player_two_decision == DECISION_STEAL) {
                coin::transfer<AptosCoin>(&resource_signer, game.player_two.player_address, prize_pool_amount);
            } else if (player_one_decision == DECISION_STEAL && player_two_decision == DECISION_SPLIT) {
                coin::transfer<AptosCoin>(&resource_signer, game.player_one.player_address, prize_pool_amount);
            } else if (player_one_decision == DECISION_STEAL && player_two_decision == DECISION_STEAL) {
                coin::transfer<AptosCoin>(&resource_signer, @overmind, prize_pool_amount);
            };

            event::emit_event(&mut state.conclude_game_events, ConcludeGameEvent{
                game_id,
                player_one_decision,
                player_two_decision,
                prize_pool_amount,
    ...
    */
    public entry fun release_funds_after_expiration(_account: &signer, game_id: u128) acquires State {
        ...
        if (player_one_decision == DECISION_NOT_MADE && player_two_decision == DECISION_NOT_MADE) {
            coin::transfer<AptosCoin>(&resource_signer, @overmind, prize_pool_amount);
        } else if (player_one_decision == DECISION_NOT_MADE) {
            coin::transfer<AptosCoin>(&resource_signer, game.player_two.player_address, prize_pool_amount);
        } else if (player_two_decision == DECISION_NOT_MADE) {
            coin::transfer<AptosCoin>(&resource_signer, game.player_one.player_address, prize_pool_amount);
        };

        // TODO: Emit `ReleaseFundsAfterExpirationEvent` event
        event::emit_event(&mut state.release_funds_after_expiration_events, ReleaseFundsAfterExpirationEvent{
            game_id,
            player_one_decision,
            player_two_decision,
            prize_pool_amount,
    ...
    */
    inline fun make_decision(player_data: &mut PlayerData, salt: &String): u64 {
        ...
        check_if_hash_is_correct(*option::borrow(&player_data.salt_hash), *salt_bytes);
        ...
        let hash_value = bcs::to_bytes(&DECISION_SPLIT);
        vector::append(&mut hash_value, *salt_bytes);
        let split_decision_hash = hash::sha3_256(hash_value);

        // TODO: Create a SHA3_256 hash of a steal decision from a vector containing serialized DECISION_STEAL const
        //      and bytes of the salt
        hash_value = bcs::to_bytes(&DECISION_STEAL);
        vector::append(&mut hash_value, *salt_bytes);
        let steal_decision_hash = hash::sha3_256(hash_value);
        ...
        let player_decision_hash = option::borrow(&player_data.decision_hash);
        let decision= DECISION_NOT_MADE;
        if (split_decision_hash == *player_decision_hash) {
            decision = DECISION_SPLIT;
        } else if (steal_decision_hash == *player_decision_hash) {
            decision = DECISION_STEAL;
        };
    ...
    */
    ...
    inline fun check_if_player_participates_in_the_game(player: &signer, game: &Game) {
        ...
        let player_address = signer::address_of(player);
        assert!(game.player_one.player_address == player_address || game.player_two.player_address == player_address, EPlayerDoesNotParticipateInTheGame);
    }
    ...
    inline fun check_if_both_players_have_a_decision_submitted(game: &Game) {
        ...
        assert!(option::is_some(&game.player_one.decision_hash) && option::is_some(&game.player_two.decision_hash), EBothPlayersDoNotHaveDecisionsSubmitted);
    }
