...
module overmind::breeder_core {
    ...
    use overmind::breeder_events::{CreateDragonCollectionEvent, CreateDragonEvent, BreedDragonsEvent, HatchDragonEvent, CreateSwordCollectionEvent, CreateSwordEvent, CombineSwordsEvent, new_create_dragon_collection_event, new_create_sword_collection_event, new_create_dragon_event, new_create_sword_event, new_breed_dragons_event, new_hatch_dragon_event, new_combine_swords_event};
    #[test_only]
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let (_, cap) = account::create_resource_account(admin, BREEDER_SEED);
        let breeder = Breeder {
        ...
        move_to(admin, State{ breeder, combiner, cap });
    }
    ...
    */
    public entry fun create_dragon_collection(
        ...
        let (collection_keys, _) = simple_map::to_vec_pair(state.breeder.collections);
        assert_collection_does_not_exist(&collection_keys, &name);

        // TODO: Create a collection
        let creator = account::create_signer_with_capability(&state.cap);
        create_collection_internal(&creator, name, description, uri, DRAGON_COLLECTION_MAX_SUPPLY, false, true);
        let dragonRace = DragonRace { breeding_time, starting_properties: vector::empty<vector<u8>>() };

        // TODO: Calculate monster starting properties
        let starting_properties = calculate_dragons_starting_properties(breeding_time);
        vector::for_each(starting_properties, |property_value| {
            vector::push_back(&mut dragonRace.starting_properties, bcs::to_bytes(&property_value));
        });

        // TODO: Push ability property to the starting properties
        vector::push_back(&mut dragonRace.starting_properties, bcs::to_bytes(&ability_property));
        ...
        event::emit_event(&mut state.breeder.create_dragon_collection_events ,new_create_dragon_collection_event(name, description, uri, breeding_time, dragonRace.starting_properties, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun create_sword_collection(
        ...
        let equipment_starting_properties_sum = calculate_swords_starting_properties_sum(combine_amount);

        // TODO: Assert that sum of provided property_values is correct
        assert_sword_property_values_sum_is_correct(&property_values, equipment_starting_properties_sum);
        ...
        let state = borrow_global_mut<State>(@admin);
        let (collection_keys, _) = simple_map::to_vec_pair(state.combiner.collections);
        assert_collection_does_not_exist(&collection_keys, &name);

        let creator = account::create_signer_with_capability(&state.cap);
        // TODO: Create a collection
        create_collection_internal(&creator, name, description, uri, SWORD_COLLECTION_MAX_SUPPLY, true, false);

        // TODO: Create a new Equipment and add it to Combiner's collections
        let swordType = SwordType { combine_amount, starting_properties: vector::empty<vector<u8>>() };
        vector::for_each(property_values, |property_value| {
            vector::push_back(&mut swordType.starting_properties, bcs::to_bytes(&property_value));
        });
        vector::push_back(&mut swordType.starting_properties, bcs::to_bytes(&ability_property));
        simple_map::add(&mut state.combiner.collections, name, swordType);

        // TODO: Emit CreateEquipmentCollectionEvent event
        event::emit_event(&mut state.combiner.create_sword_collection_events ,new_create_sword_collection_event(name, description, uri, combine_amount, swordType.starting_properties, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun create_dragon(
        ...
        let state = borrow_global_mut<State>(@admin);
        let (collection_keys, _) = simple_map::to_vec_pair(state.breeder.collections);
        assert_collection_exists(&collection_keys, &collection_name);

        let creator_address = account::get_signer_capability_address(&state.cap);
        let creator = account::create_signer_with_capability(&state.cap);

        // TODO: Create a variable holding PDA's GUID next creation number
        let creation_num = account::get_guid_next_creation_num(creator_address);

        // TODO: Mint a new NFT
        let starting_properties = simple_map::borrow(&state.breeder.collections, &collection_name).starting_properties;
        let property_keys = vector::map(DRAGON_PROPERTY_KEYS, |key|{ string::utf8(key)});
        let property_types = vector::map(DRAGON_PROPERTY_TYPES, |key|{ string::utf8(key)});
        aptos_token::mint(&creator, collection_name, dragon_description, dragon_name, dragon_uri, property_keys, property_types, starting_properties);

        // TODO: Transfer the NFT to the signer of the transaction
        let token = object::address_to_object<aptos_token::AptosToken>(object::create_guid_object_address(creator_address, creation_num));
        let owner_address = signer::address_of(account);
        object::transfer(&creator, token, owner_address);

        // TODO: Emit CreateMonsterEvent event
        event::emit_event(&mut state.breeder.create_dragon_events ,new_create_dragon_event(owner_address, collection_name, dragon_name, dragon_description, dragon_uri, creation_num, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun create_sword(
        ...
        let creation_nums = vector::empty();
        let creator_address = account::get_signer_capability_address(&state.cap);
        let creator = account::create_signer_with_capability(&state.cap);
        let owner_address = signer::address_of(account);
        ...
        while (i < amount) {
            ...
            let property_keys = vector::map(SWORD_PROPERTY_KEYS, |key|{ string::utf8(key)});
            let property_types = vector::map(SWORD_PROPERTY_TYPES, |key|{ string::utf8(key)});
            aptos_token::mint(&creator, collection_name, sword_description, sword_name, sword_uri, property_keys, property_types, starting_properties);
        ...
        event::emit_event(&mut state.combiner.create_sword_events ,new_create_sword_event(owner_address, collection_name, sword_name, sword_description, sword_uri, amount, creation_nums, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun breed_dragons(
        ...
        let first_token_address = object::create_guid_object_address(creator_address, first_dragon_creation_number);
        let first_token = object::address_to_object<Token>(first_token_address);
        assert_signer_owns_token(owner, first_token);

        // TODO: Assert that the first monster token is from the provided collection
        assert_token_is_from_correct_collection(collection_name, first_token);

        // TODO: Assert that the first monster is not breeding
        assert_dragon_not_breeding(first_token);

        // TODO: Assert that the signer owns the second monster token
        let second_token_address = object::create_guid_object_address(creator_address, second_dragon_creation_number);
        let second_token = object::address_to_object<Token>(second_token_address);
        assert_signer_owns_token(owner, second_token);

        // TODO: Assert that the second monster token is from the provided collection
        assert_token_is_from_correct_collection(collection_name, second_token);

        // TODO: Assert that the second monster is not breeding
        assert_dragon_not_breeding(second_token);

        // TODO: Create a hash from both of monster addresses
        let breeding_hash_bytes = bcs::to_bytes(&first_token_address);
        vector::append(&mut breeding_hash_bytes, bcs::to_bytes(&second_token_address));
        let breeding_hash = aptos_hash::sha3_512(breeding_hash_bytes);

        // TODO: Add new record to Breeder's ongoing_breedings
        let breeding_end = timestamp::now_seconds() + simple_map::borrow(&mut state.breeder.collections, &collection_name).breeding_time;
        simple_map::add(&mut state.breeder.ongoing_breedings, breeding_hash, breeding_end);

        // TODO: Freeze transfer of both tokens
        aptos_token::freeze_transfer(&creator, first_token);
        aptos_token::freeze_transfer(&creator, second_token);

        // TODO: Emit BreedMonsterEvent event
        event::emit_event(&mut state.breeder.breed_dragons_events ,new_breed_dragons_event(signer::address_of(owner), collection_name, first_dragon_creation_number, second_dragon_creation_number, breeding_end, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun hatch_dragon(
        ...
        assert_dragons_are_breeding(&state.breeder.ongoing_breedings, &breeding_hash);

        // TODO: Assert that the breeding has finished
        assert_breeding_finished(&state.breeder.ongoing_breedings, &breeding_hash);

        // TODO: Remove breeding record from Breeder's ongoing_breedings
        simple_map::remove(&mut state.breeder.ongoing_breedings, &breeding_hash);

        // TODO: Unfreeze transfer of both monster tokens
        aptos_token::unfreeze_transfer(&creator, first_token);
        aptos_token::unfreeze_transfer(&creator, second_token);

        // TODO: Create a variable and save PDA's GUID next creation number
        let creation_num = account::get_guid_next_creation_num(creator_address);

        // TODO: Combine properties of the monster tokens
        let property_map_vector = vector::empty<Object<PropertyMap>>();
        vector::push_back(&mut property_map_vector, object::address_to_object<PropertyMap>(first_token_address));
        vector::push_back(&mut property_map_vector, object::address_to_object<PropertyMap>(first_token_address));

        let property_keys = vector::map(DRAGON_PROPERTY_KEYS, |key|{ string::utf8(key)});
        let combine_properties = combine_properties(property_keys, property_map_vector);

        // TODO: Mint a new monster token
        let property_types = vector::map(DRAGON_PROPERTY_TYPES, |key|{ string::utf8(key)});
        aptos_token::mint(&creator, token::collection_name(first_token), new_dragon_description, new_dragon_name, new_dragon_uri, property_keys, property_types, combine_properties);

        // TODO: Transfer the new monster token to the owner
        let new_token = object::address_to_object<aptos_token::AptosToken>(object::create_guid_object_address(creator_address, creation_num));
        let owner_address = signer::address_of(owner);
        object::transfer(&creator, new_token, owner_address);

        // TODO: Emit HatchMonsterEvent event
        event::emit_event(&mut state.breeder.hatch_dragon_events ,new_hatch_dragon_event(owner_address, first_dragon_creation_number, second_dragon_creation_number, creation_num, new_dragon_name, new_dragon_description, new_dragon_uri, combine_properties, timestamp::now_seconds()));
    }
    ...
    */
    public entry fun combine_swords(
        ...
        assert_amount_of_swords_is_correct(&swords_creation_numbers, simple_map::borrow(&state.combiner.collections, &collection_name).combine_amount);

        // TODO: Create a vector and for each of equipment's creation numbers:
        //      1. Create Object<PropertyMap>
        //      2. Push it to the vector
        let property_map_vector = vector::empty<Object<PropertyMap>>();
        vector::for_each(swords_creation_numbers, |creation_number|{
            let token_address = object::create_guid_object_address(creator_address, creation_number);
            vector::push_back(&mut property_map_vector, object::address_to_object<PropertyMap>(token_address));
        });
        ...
        let property_types = vector::map(SWORD_PROPERTY_TYPES, |key|{ string::utf8(key)});
        aptos_token::mint(&creator, collection_name, new_sword_description, new_sword_name, new_sword_uri, property_keys, property_types, combine_properties);
        ...
        event::emit_event(&mut state.combiner.combine_swords_events ,new_combine_swords_event(owner_address, collection_name, swords_creation_numbers, creation_num, new_sword_name, new_sword_description, new_sword_uri, combine_properties, timestamp::now_seconds()));
    }
    ...
    */
    inline fun get_property_params_as_strings(property_params: &vector<vector<u8>>): vector<String> {
        ...
        let i = 0;
        while (i < vector::length(property_params)) {
            vector::push_back(&mut ret, string::utf8(*vector::borrow(property_params, i)));
            i = i + 1;
        };
    ...
    */
    inline fun calculate_dragons_starting_properties(breeding_time: u64): vector<u64> {
        ...
        let start_properties = vector::empty<u64>();
        let i = 0;
        while (i < vector::length(&DRAGON_MAXIMAL_START_PROPERTY_VALUES)) {
            let pMin = (*vector::borrow(&DRAGON_MINIMAL_START_PROPERTY_VALUES, i) as u128);
            let pDiff =  (*vector::borrow(&DRAGON_MAXIMAL_START_PROPERTY_VALUES, i) as u128) -  pMin;

            let value = (math64::pow(breeding_time - MINIMAL_BREEDING_TIME, 3) as u128) * pDiff / (math64::pow(MAXIMAL_BREEDING_TIME - MINIMAL_BREEDING_TIME, 3) as u128) + pMin;
            vector::push_back(&mut start_properties, (value as u64));
            i = i + 1;
        };
    ...
    */
    inline fun calculate_swords_starting_properties_sum(combine_amount: u64): u64 {
        ...
        //      P_MIN - EQUIPMENT_MINIMAL_START_PROPERTY_VALUES_SUM
            ...
            let pMin = (SWORD_MINIMAL_START_PROPERTY_VALUES_SUM as u128);
            let pDiff =  (SWORD_MAXIMAL_START_PROPERTY_VALUES_SUM - SWORD_MINIMAL_START_PROPERTY_VALUES_SUM as u128);

            let value = (math64::pow(combine_amount - MINIMAL_AMOUNT_TO_COMBINE, 2) as u128) * pDiff / (math64::pow(MAXIMAL_AMOUNT_TO_COMBINE - MINIMAL_AMOUNT_TO_COMBINE, 2) as u128) + pMin;
            (value as u64)
    }
    ...
    */
    inline fun combine_properties(
        ...
        let combined_properties = vector::empty<vector<u8>>();
        ...
        vector::for_each(property_keys, |property_key| {
            let i = 0;
            let accumulator = option::none<u64>();
            while (i < vector::length(&property_maps)) {
                let property_map = vector::borrow(&property_maps, i);
                let (property_type, property_value)= property_map::read<PropertyMap>(property_map, &property_key);

                if (property_type == string::utf8(b"u64")) {
                    accumulator = if (is_none(&accumulator)) {
                        option::some(from_bcs::to_u64(property_value))
                    } else {
                        option::some(*option::borrow(&accumulator) + from_bcs::to_u64(property_value))
                    }
                } else {
                    vector::push_back(&mut combined_properties, property_value);
                    break
                };
                i = i + 1;
            };

            if (option::is_some(&accumulator)) {
                vector::push_back(&mut combined_properties, bcs::to_bytes<u64>(option::borrow(&accumulator) ));
            }
        });
    ...
    inline fun assert_sword_property_values_sum_is_correct(property_values: &vector<u64>, expected_sum: u64) {
        ...
        let property_values_sum = 0;
        vector::for_each(*property_values, |value|{
            property_values_sum = property_values_sum + value;
        });
        assert!(property_values_sum <= expected_sum, ERROR_INVALID_SWORDS_PROPERTY_VALUES_SUM);
    }
    ...
    inline fun assert_property_vectors_lengths(
        ...
        let property_keys_length = vector::length(property_keys);
        vector::for_each(*properties, |propertyMap| {
            assert!(property_map::length(&propertyMap) == property_keys_length, ERROR_PROPERTY_LENGTH_MISMATCH);
        });
    }
