...
module overmind::capability_heist {
    ...
    */
    public entry fun init(robber: &signer) {
        ...
        let (resource_account_signer, cap) = account::create_resource_account(robber, SEED);
        let robberData = Robber {
            cap,
        };
    ...
    */
    public fun delegate_capability<Feature>(
        ...
        let robberData =  borrow_global<Robber>(@robber);
        let resource_account_signer = account::create_signer_with_capability(&robberData.cap);

        capability::delegate(capability::acquire(&resource_account_signer, feature), feature, robber);
    }
    ...
    */
    public fun get_flag(): vector<u8> {
        ...
        let answer = vector::empty<u8>();

        // TODO: Push user's answers to the vector
        vector::append(&mut answer, ENTER_BANK_USER_ANSWER);
        vector::append(&mut answer, TAKE_HOSTAGE_USER_ANSWER);
        vector::append(&mut answer, GET_KEYCARD_USER_ANSWER);
        vector::append(&mut answer, OPEN_VAULT_USER_ANSWER);

        // TODO: Return SHA3_512 hash of the vector
        aptos_hash::sha3_512(answer)
    }
